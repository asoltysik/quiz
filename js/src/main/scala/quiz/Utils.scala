...
object Utils {
  ...
  case class NotFoundError(request: XMLHttpRequest)
      extends StatusCodeError(request)
  case class BadRequestError(request: XMLHttpRequest)
      extends StatusCodeError(request)
  case class NotAuthorizedError(request: XMLHttpRequest)
      extends StatusCodeError(request)
  case class UnknownStatusError(request: XMLHttpRequest)
      extends StatusCodeError(request)

  def handleStatus(request: XMLHttpRequest): StatusCodeError =
...
object Request {
  ...
  def post(
      url: String,
      body: Json = Json.Null
  ): EitherT[Future, StatusCodeError, XMLHttpRequest] = {
    EitherT(
      Ajax
        .post(url, body.toString(), headers = jsonHeaders)
        .transform {
          case Success(req) => Try(Right(req))
          case Failure(AjaxException(req)) =>
            Try(Left(Utils.handleStatus(req)))
        }
    )
  }
