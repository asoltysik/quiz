...
class QuizActor extends Actor {
  def receive: Receive = active(Uninitialized(), Map.empty[Question, (String, Boolean)])
  ...
  def active(quizState: Uninitialized, answeredMap: Map[Question, (String, Boolean)]): Receive = {
    case Starting(userId, quizId) =>
      ...
      context become active(QuizData(quiz, LocalDateTime.now()), Map.empty[Question, (String, Boolean)])
  }

  def active(quizState: QuizData, answeredMap: Map[Question, (String, Boolean)]): Receive = {
    case Answering(userId, questionId, answerText) =>
      ...
      context become active(quizState, answeredMap.updated(question, (answerText, correct)))
    case Finishing(userId, quizId) =>
      val results = getFinalResults(quizState, answeredMap)
      sender() ! FinishResponse(results)
  }

  private def getFinalResults(quizState: QuizData, answeredMap: Map[Question, (String, Boolean)]) = {
